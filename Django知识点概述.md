...
3. 生成CSV / Excel / PDF / 统计报表。
   ...
   - 大文件的流式处理：`StreamingHttpResponse`。
     ...
     > 说明：如果需要生成PDF文件，可以需要安装`reportlab`。
...
问题2：表单上传文件时应该注意哪些问题？（表单的设置、多文件上传、图片预览（FileReader）、Ajax上传文件、上传后的文件如何存储、调用云存储（OSS、七牛、Bmob、LeanCloud））
...
> 说明：上面使用了drf_extensions提供的CacheResponseMixin混入类实现了对接口数据的缓存。如果重写了获取数据的方法，可以使用drf_extensions提供的@cache_response来实现对接口数据的缓存，也可以用自定义的函数来生成缓存中的key。
...
1. RBAC - 基于角色的访问控制（用户-角色-权限[-资源]，实体之间通常都是多对多关系）。
...
2. 加密和解密（对称加密和非对称加密）
   ...
   >>> str2
   b'\xb7\xa0\xe1\xbd\xb8N\xd1@l\xe6\xabqI\x05\xb9'
   >>>
   >>> str3 = AES.new(key, AES.MODE_CFB, iv).decrypt(str2)
   >>> str3.decode()
   '我爱你们！'
   ```
   ...
   >>> message2
   b'LCOlC6Y4WeHSIVqMvK9h3wQpZVlBlv2+4KBElcUb6rbCh6DqRfakF6JOEP0RsBVW7x3O+Js+1nREEmXoArPiiLkDL4TOstnRupZPqV3qlNDqSh421dfXXt8sK+3xA1/4di1by36zpuyKIxYEkYH4HXW+Ff7+5S5wt05QGlcc97o='
   >>>
